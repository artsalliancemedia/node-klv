...
/**
 * Creates a KLV streaming parser. Options are:
 *
 * keyLength: specify the length of the KLV key
 * emitData: if true then values will be emitted as data events
 */
var KLVStream = function(options) {
    ...
    // Check whether KLV values should be emitted as data
    this.emitData = Boolean(options && options.emitData);
...
KLVStream.prototype.write = function(data) {
    ...
    while(!done) {
        ...
        // Emit the length
            ...
            if (typeof decodedBER.value === 'undefined') { return true; }

...
exports.createStream = function() {
	return new KLVStream();
};

/**
 * Creates a KLV streaming parser
 */
var KLVStream = function() {
	this.writable = true;
	// this.readable = true;
...
KLVStream.prototype.write = function(data) {
	...
	while(!done) {
		// Emit the key
		if (this.state === KLV_STATES.KEY && this.buffer.length >= KEY_LENGTH) {
			var key = this.buffer.slice(0, 16);
			this.buffer = this.buffer.slice(16); // Remove the key portion from the buffer
